...
- Чтобы понять, что у вас есть проблемы с асинхронным кодом, попробуйте внутри теста расставить вызовы `setTimeout`. Это замедлит тест и отсрочит вызовы проверок, тем самым позволив отработать асинхронному коду. Перед пушем не забудьте удалить эти вызовы. Деплоить такие тесты нельзя.
